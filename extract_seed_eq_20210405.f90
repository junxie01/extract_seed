! 2016-10-26 -- extract three components of sac file from seeds file or miniseed file
! 2021-03-30 -- rmove temporary files
! 2021-03-30 -- add the original time
! 2021-03-31 -- use the same parameter file with fetch_eq
! 2021-04-02 -- 
! 2021-04-05 -- add the distance
...
integer::            year_b,day_b,year_e,day_e,dhour,swap
...
call getarg(1,para)
open( 90,file=para)
read( 90,'(a80)')sta_list
read( 90,'(a80)')eq_list
read( 90,*)seed_type,dsec,icom,co,do_resp
read( 90,*)f1,f2,do_rm_response,do_decimate
read( 90,'(a80)')dir_seed
read( 90,'(a80)')dir_resp
read( 90,'(a80)')dir_mdata
read( 90,'(a80)')dir_sac
close(90)
if(icom.eq.1)then
   com(1)=co
...
open(50,file=sta_list)
do i=1,nstmax
   read(50,*,end=12,err=12)net(i),sta(i)
enddo
12  close(50)
nsta=i-1
!write(*,*)'Number of station is ',nsta
write(command,'("mkdir -p",1x,1a,1x,"2>/dev/null")')trim(dir_sac)
open(60,file=eq_list)
do ieq=1,neqmax                                  ! loop over earthquake catalog
   read(60,*,end=14,err=14)year,month,day,hour,mmin,sec,msec,evla,evlo,evdp,mag,dist
   ...
   if(seed_type.eq.1)then          ! if it is the seed file
      ...
      write(command,'("mkdir -p ",1a,"/",i4.4,"_",i2.2,"_",i2.2,"_",i2.2,"_",i2.2,"_",i2.2,"_",i3.3)')&
      trim(dir_sac),year,month,day,hour,mmin,sec,msec
      call system(command)  ! make directory
      do is=1,nsta          ! loop over station
         do ic=1,icom       ! loop over component
            write(saclist,'(i4.4,"_",i2.2,"_",i2.2,"_",i2.2,"_",i2.2,"_",i2.2,"_",i3.3,"_",1a,"_",1a,"_",1a,".list")')&
            year,month,day,hour,mmin,sec,msec,trim(net(is)),trim(sta(is)),trim(com(ic))
            ...
            write(sac,'(1a,"/",i4.4,"_",i2.2,"_",i2.2,"_",i2.2,"_",i2.2,"_",i2.2,"_",i3.3,"/",1a,"_",1a,"_",1a,".SAC")')&
            trim(dir_sac),year,month,day,hour,mmin,sec,msec,trim(net(is)),trim(sta(is)),trim(com(ic))
            inquire(file=sac,exist=ext)
            !write(*,*)trim(saclist),trim(sac_tmp),trim(sac)
            if(ext)cycle
            open( 20,file=bash) 
            write(20,'("#!/bin/bash")')
            write(20,'("rm",1x,1a,1x,"2>/dev/null")')trim(saclist)
            ...
            do ifile=1,nfmax
               ...
               if(imark==0)then
                  ...
                  sacheado%mag=mag
                  dday=0
                  do iy=sacheado%nzyear,year
                     dayb=sacheado%nzjday
                     if(iy.ne.sacheado%nzyear)dayb=1
                     daye=day_e
                     if(iy.ne.year)daye=jday
                     do id=dayb,daye-1
                        dday=dday+1
                     enddo
                  enddo
                  !write(*,*)sacheado%nzyear,year
                  !write(*,*)sacheado%nzjday,nzjday
                  !write(*,*)dday
                  dtt=dday*86400.0+(hour-sacheado%nzhour)*3600.0+(mmin-sacheado%nzmin)*60.0+&
                          (sec-sacheado%nzsec)+(msec-sacheado%nzmsec)/1000.0
                  sacheado%nzyear=year
                  sacheado%nzjday=nzjday
                  sacheado%nzhour=hour
                  sacheado%nzmin=mmin
                  sacheado%nzsec=sec
                  sacheado%nzmsec=msec
                  sacheado%b=sacheado%b-dtt
                  sacheado%o=0
                  sacheado%dist=dist
                  sacheado%LCALDA=.false.
               endif
               !write(*,*)'hello there1'
...
!      ******************************************** remove response before paste
               if(do_rm_response.eq.1)then
                  ...
                  write(resp,'(1a,"/",i4.4,"_",i2.2,"_",i2.2,"/RESP.",1a,".",1a,".",1a,".",1a)')&
                  trim(dir_resp),year,month,day,trim(net(is)),trim(sta(is)),trim(kho),trim(com(ic))
                  if(kho.eq."-1")write(resp,'(1a,"/",i4.4,"_",i2.2,"_",i2.2,"/RESP.",1a,".",1a,"..",1a)')&
                          trim(dir_resp),year,month,day,trim(net(is)),trim(sta(is)),trim(com(ic))
                  !write(*,*)'remove response ',trim(resp), " from ",trim(file)
