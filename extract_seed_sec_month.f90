...
integer is,ih,iy,id,dayb,daye,jday,nh,nnh,nhh1,icom,month,im
integer,dimension(12) :: monthday=(/31,28,31,30,31,30,31,31,30,31,30,31/)
...
iy=year_b
if(mod(iy,4).eq.0.and.mod(iy,100).ne.0.or.mod(iy,400).eq.0)monthday(2)=29

dayb=0
do im=1,month-1
   dayb=dayb+monthday(im)
enddo
daye=dayb+monthday(month)
