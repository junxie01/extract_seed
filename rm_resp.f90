...
!write(20,'("trans from evalresp fname",1x,1a,1x,"to none freq",4f15.6)')trim(resp),f1,f2,f3,f4
...
call system(command)
open(31,file=num_out)
read(31,*)num
close(31)
if(num.ge.1)then
   write(*,'("There is something wrong with response file of",1x,1a)')trim(sac)
   err=-1 
endif
write(command,'("rm",1x,1a)')trim(output); call system(command)
write(command,'("rm",1x,1a)')trim(num_out);call system(command)
return
end subroutine
